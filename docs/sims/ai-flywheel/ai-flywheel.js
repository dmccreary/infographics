...
let descriptions = {
  'Data': 'The foundation of any AI system. This includes training datasets, real-world usage data, user interactions, and feedback. Quality and quantity of data directly impact model performance. As the flywheel spins, more data is continuously collected from user interactions.',
  'Model': 'The AI algorithm or machine learning model trained on the data. This could be a neural network, decision tree, or other ML algorithm. Better data leads to better models, which can make more accurate predictions and decisions.',
  'Prediction': 'The output or decision made by the AI model. This could be recommendations, classifications, forecasts, or automated actions. Better models produce more accurate and useful predictions that create value for users.',
  'Feedback': 'User responses, behaviors, and outcomes resulting from the predictions. This includes explicit feedback (ratings, clicks) and implicit feedback (usage patterns, behaviors). Feedback validates model performance and provides new training data.'
};
...
function drawDescription() {
  ...
  if (currentHover) {
    ...
    let hoveredNode = nodes.find(n => n.id === currentHover);

    let label = hoveredNode.label + ':';
    ...
    text(label, leftMargin, descriptionY);

    // Draw the description indented by the label width so all lines align
    let textX = leftMargin + labelWidth + 6;
    let textW = descWidth - labelWidth - 6;
    textStyle(NORMAL);
    textSize(baseFontSize);
    fill('black');
    text(description, textX, descriptionY + 2, textW, descriptionHeight);
  } else {
